...
func (user *UserDBData) getByID() (int, error) {
	if err := verifyDatabaseConnection(dbClient); err != nil {
		return http.StatusInternalServerError, fmt.Errorf("unable to Perform %s Operation on Table: %s. %v", "Read", UserTableName, err)
	}
...
func validateUserModel(userModel *UserModel, create bool) error {
	...
	if userModel.Address.AddressLine1 == "" {
		return fmt.Errorf("invalid UserModel. Empty Address Line 1")
	}
	if userModel.Address.City == "" {
		return fmt.Errorf("invalid UserModel. Empty City")
	}
	if userModel.Address.State == "" {
		return fmt.Errorf("invalid UserModel. Empty State")
	}
	if userModel.Address.Country == "" {
		return fmt.Errorf("invalid UserModel. Empty Country")
	}
	if userModel.Address.Pincode == "" {
		return fmt.Errorf("invalid UserModel. Empty Pincode")
	}
