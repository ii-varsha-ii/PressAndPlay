...
func newRedisHandler() (*redis.Client, error) {
	host := getEnv(REDIS_HOST_ENV, "localhost")
	port := getEnv(REDIS_PORT_ENV, "6379")
	password := getEnv(REDIS_PASSWORD_ENV, "admin")
	dbName, _ := strconv.Atoi(getEnv(REDIS_DB_ENV, "0"))
...
func createNewSession(userData *UserDBData) (string, error) {
	...
	expiration, _ := strconv.Atoi(getEnv(REDIS_DATA_EXPIRATION_IN_HRS_ENV, "0"))
