...
func newPostgresClient() (*bun.DB, error) {
	host := getEnv(POSTGRES_HOST_ENV, "localhost")
	port := getEnv(POSTGRES_PORT_ENV, "5432")
	username := getEnv(POSTGRES_USERNAME_ENV, "admin")
	password := getEnv(POSTGRES_PASSWORD_ENV, "admin")
	dbName := getEnv(POSTGRES_DB_ENV, "pressandplay")
